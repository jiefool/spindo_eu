<?php
  ...
  // $ipAddress = $_SERVER['REMOTE_ADDR'];
   $ipAddress = "46.102.103.152";

  $clientLocation = unserialize(file_get_contents('http://www.geoplugin.net/php.gp?ip='.$ipAddress));
  $clientContinent =  $clientLocation["geoplugin_continentCode"];
  $clientContinentcountries = json_decode(file_get_contents('https://restcountries.eu/rest/v1/currency/eur'));
  $clientCountryName =$clientLocation["geoplugin_countryName"];
  $clientCountryCode = $clientLocation["geoplugin_countryCode"];
  $clientCityName = $clientLocation["geoplugin_city"];

  $clientCityCountryText = "";
  if ($clientCityName != ""){
    $clientCityCountryText .= $clientCityName;
    $clientCityCountryText .= ", ";
  }
  $clientCityCountryText .= $clientCountryName;

  $defineCities = ["Tallinn", "Tartu","Narva","Viljandi","Rakvere","Parnu","Haapalu","Voru","Jogeva","Kureaare","Ventpil","Jurmala","Jelgava","Riga","Liepaja","alapil","Ogre","Valmiera","Alukne","Jekabpil","Daugavpil","Alytu","Anykciai","Kauna","Klaipeda","Panevezy","iauliai","ilute","Teliai","Varena","Vilniu"];

  // $citiesCsv = array_map('str_getcsv', file('./worldcities.csv'));
  // $countryCities = getCitiesForCountry($clienCountryCode, $citiesCsv);

  // function getCitiesForCountry($countryCode, $citiesCsv){
  //   $citiesCountry = array();
  //   foreach($citiesCsv as $countryCities){
  //    if ($countryCities[0]==$countryCode){
  //       array_push($citiesCountry, $countryCities[6]);
  //     }
  //   }
            ...
            <tr>
                ...
                <table id="form" cellspacing="2" cellpadding="2" border="0">
                    <tbody>
                        ...
                        <tr>
                            <td valign="top" width="150" align="right">I live in or near</td>
                            <td><select id="city" onchange="change_country()" name="city">
                            <?php
                                foreach($defineCities as $city){
                                    $isSelected = "";
                                    if ($city == $clientCityName){
                                        $isSelected = "selected";
                                    }
                           ...
                           <!--  <option value="Tallinn" selected="selected">Tallinn</option>
                            ...
                            <div id="form_small_txt">We've detect your location to <?php echo $clientCityCountryText ?>. You can changed it if not correct.</div>
                            </td>
                        </tr>
                        <tr>
