...
class Weglot {
	...
	public function init_function() { 
		...
		if(!is_admin() || (is_admin() && strpos($this->request_uri, 'page=Weglot') !== false)) {
			// Add JS
			wp_register_script('wp-weglot-js', WEGLOT_RESURL.'wp-weglot-js.js', false,false, false);
			wp_enqueue_script('wp-weglot-js');

			//Add CSS
			wp_enqueue_style( 'font-awesome', '//maxcdn.bootstrapcdn.com/font-awesome/latest/css/font-awesome.min.css' );
			wp_register_style('wp-weglot-css', WEGLOT_RESURL.'wp-weglot-css.css', false,false, false);
			...
			if(is_admin()) {
				// Add Admin JS
				wp_register_script('wp-weglot-admin-js', WEGLOT_RESURL.'wp-weglot-admin-js.js', array('jquery'),false, true);
				wp_enqueue_script('wp-weglot-admin-js');

				//Add Admin CSS
				wp_register_style('wp-weglot-admin-css', WEGLOT_RESURL.'wp-weglot-admin-css.css', false,false, false);
	...
	public function plugin_menu() {
		...
		if(isset($_GET['settings-updated']) && $_GET['settings-updated'] && strpos($this->request_uri, 'page=Weglot') !== false)
		{
			...
			$accepted = array("sq","en","ar","hy","az","af","eu","be","bg","bs","vi","hu","ht","nl","el","ka","da","he","id","ga","it","is","es","kk","ca","ky","zh","ko","lv","lt","mg","ms","mt","mk","mn","de","no","fa","pl","pt","ro","ru","sr","sk","sl","sw","tg","th","tr","uz","uk","fi","fr","hr","cs","sv","et","ja");
			foreach($d as $k=>$l) {
	...
	public function treatPage($final) {
		...
		if(!is_admin() && strpos($request_uri,'wc-ajax') === false && $this->original_l!="" && $this->destination_l!="") {
			...
			if($this->isEligibleURL($url) && WGUtils::is_HTML($final)) {
				...
				if (strpos($final,'weglot_switcher') === false) {
	...
	public function getHomeDirectory() {
		...
		if($opt_siteurl!="" && $opt_home!="" && $opt_siteurl==$opt_home) {
			if( (substr($opt_siteurl,0,7) == "http://" && strpos(substr($opt_siteurl,7),'/') !== false) ||  (substr($opt_siteurl,0,8) == "https://" && strpos(substr($opt_siteurl,8),'/') !== false) ) {
				return $opt_siteurl;
			}
		}
		return null;
	...
	public function returnWidgetCode() { 
		...
		foreach($destEx as $d) { 
			if($d!=$current) {
				...
				$list .= '<li class="'.$d.'"><a wg-notranslate href="'.$link.'">'.($withname ? ($full? WGUtils::getLangNameFromCode($d,false):strtoupper($d)):"").'</a></li>';
			}
		}
		$list .= $is_dropdown ? "</ul>":"";	
		$tag =  $is_dropdown ? "div":"li";
		
		$moreclass = (get_option("with_flags")=='on') ? 'wg-flags ':'';
		$moreclass .= (get_option("is_dropdown")=='on') ? 'wg-drop ':'wg-list ';
		
		$type_flags = get_option("type_flags") ? get_option("type_flags"):0;
		
		$moreclass .= $type_flags==0 ? '':'flag-'.$type_flags.' ';
		
		$button = '<!--Weglot '.WEGLOT_VERSION.'--><aside id="weglot_switcher" wg-notranslate class="'.$moreclass.'country-selector closed" onclick="openClose(this);" ><'.$tag.' class="wgcurrent '.$current.'"><a href="javascript:void(0);">'.($withname ? ($full? WGUtils::getLangNameFromCode($current,false):strtoupper($current)):"").'</a></'.$tag.'>'.$list.'</aside>';
